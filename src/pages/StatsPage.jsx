    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <div className="max-w-lg mx-auto px-4 py-6 space-y-4">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-5">
          <p className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-3">Resumen global</p>
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              <p className="text-2xl font-extrabold text-gray-900 dark:text-white">{data.length}</p>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Vehículos</p>
            </div>
            <div>
              <p className="text-2xl font-extrabold text-gray-900 dark:text-white">{totalCount}</p>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Mantenimientos</p>
            </div>
            <div>
              <p className="text-2xl font-extrabold text-emerald-600">{formatEur(totalGlobal)}</p>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Gasto total</p>
            </div>
          </div>
        </div>
        ...
        {byVehicle.length > 0 && (
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-5">
            <p className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-3">Gasto por vehículo</p>
              ...
              {byVehicle.map(({ car, cost, count }) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{count} mantenimientos</p>
                  </div>
                )
              })}
            </div>
        ...
        {topTypes.length > 0 && (
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-5">
            <p className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-3">Gasto por tipo</p>
            <div className="space-y-3">
              {topTypes.map(([type, amount]) => {
                ...
                return (
                  <div key={type}>
                    <div className="flex items-center justify-between text-sm mb-1">
                      <span className="text-gray-700 dark:text-gray-300">{t.icon} {t.label}</span>
